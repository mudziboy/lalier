...
# -----------------------------------------------------
# FIX: MEMBUAT CONFIG JSON DEFAULT
# -----------------------------------------------------
echo -e "${CYAN}➤ Membuat konfigurasi server default...${NC}"
cat > "$ZIVPN_CFG" <<EOF
{
  "listen": ":5667",
...
EOF
chmod 644 "$ZIVPN_CFG"
INFO "File config.json berhasil dibuat."
# -----------------------------------------------------
...
echo -e "${CYAN}➤ Menyiapkan ZIVPN API Server (Node + PM2)...${NC}"
loading_bar 20

# Install Node.js jika belum ada
if ! command -v node >/dev/null 2>&1; then
  INFO "Node.js belum terpasang, menginstall..."
  apt update -y
  apt install -y nodejs npm
else
  INFO "Node.js sudah terpasang"
fi

# Install PM2 jika belum ada
if ! command -v pm2 >/dev/null 2>&1; then
  INFO "PM2 belum terpasang, menginstall..."
  npm install -g pm2
else
  INFO "PM2 sudah terpasang"
fi

# Cek file API
API_FILE="/etc/zivpn/api.js"

if [ -f "$API_FILE" ]; then
  INFO "File API ditemukan: $API_FILE"

  # Hentikan jika sudah ada
  pm2 delete zivpn-api >/dev/null 2>&1 || true

  # Jalankan API
  INFO "Menjalankan ZIVPN API dengan PM2..."
  pm2 start "$API_FILE" --name zivpn-api

  # Simpan & autostart
  pm2 save
  pm2 startup systemd -u root --hp /root >/dev/null 2>&1

  INFO "ZIVPN API berhasil dijalankan dan auto-start aktif"

else
  WARN "File $API_FILE tidak ditemukan, API tidak dijalankan otomatis"
fi
